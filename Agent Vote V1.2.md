...
这是任何一次对外 demo 的最低保障，也是回归测试的基本面。V1.2 的所有新能力都以「向后兼容 + 可选开启」的方式叠加在它之上。
...
[已投 vN] --revoke--> [已撤回]    (撤回 = 软删除，仍占统计但不算票)
[任意] --deadline--> [已结算]
[已结算] --> 进入历史查阅
```
...
| `db.json`（旧） | 一次性迁移脚本 | 启动时从 V1.0 的 `db.json` 导入 SQLite 后归档 |
...
| 美国 | 接受稳定币 / USDT，遵循 CFTC 规则（参照 Kalshi） | 需要明确条款 |
